...
def getDataFromData(file):
    global total_time_left, over_time, monday, tuesday, wednesday, thursday, friday, saturday, sunday
...
def logInfo(weekday):
    ...
    if logger_timer >= logger_delay:
        if total_time_left > 0:
            logger.info('\nTotal time left: ' + timedelta(seconds=total_time_left).__str__())
        else:
            logger.info('\nOvertime: ' + timedelta(seconds=over_time).__str__())

        if weekday == 0:
            logger.info('Monday: ' + timedelta(seconds=monday).__str__())
        elif weekday == 1:
            logger.info('\nTuesday: ' + timedelta(seconds=tuesday).__str__())
        elif weekday == 2:
            logger.info('\nWednesday: ' + timedelta(seconds=wednesday).__str__())
        elif weekday == 3:
            logger.info('\nThursday: ' + timedelta(seconds=thursday).__str__())
        elif weekday == 4:
            logger.info('\nFriday: ' + timedelta(seconds=friday).__str__())
        elif weekday == 5:
            logger.info('\nSaturday: ' + timedelta(seconds=saturday).__str__())
        elif weekday == 6:
            logger.info('\nSunday: ' + timedelta(seconds=sunday).__str__())
...
def main(delay):
    global apps_list, total_time_left, over_time, time_delay, weekly_working_time
