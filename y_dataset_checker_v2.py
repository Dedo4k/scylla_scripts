...
def checking_labels_in_folder(folder):
    try:
        ...
        if len(files_to_move) != 0:
            print('Wrong files founded:', len(files_to_move))
            print('Moving wrong files to folder', 'mistake')
            for file in files_to_move:
                move_to_folder(file, folder, 'mistake')
            print('Moving files is finished.')
    except Exception:
        print('Unknown exception occurred. Folder:', folder)
...
def checking_small_boxes_in_folder(folder):
    try:
        ...
        if len(files_to_move) != 0:
            print('Wrong files founded:', len(files_to_move))
            print('Moving wrong files to folder', '2small')
            for file in files_to_move:
...
def checking_missing_images(folder):
    try:
        ...
        for file in [y for x in os.walk(folder) for y in glob(os.path.join(x[0], '*.xml*'))]:
            ...
            if not os.path.isfile(png_name) and not os.path.isfile(jpg_name) and not os.path.isfile(jpeg_name):
