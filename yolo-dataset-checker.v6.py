...
pred_labels = ['car', 'person', 'truck', 'bus', 'motorcycle', 'bicycle', 'hhelmet', 'dog', 'helmet', 'rifle', 'bird','van', 'pickup', 'cat']
bad_classes = ['sniper']
...
def remove_empty_xmls(folder):
    print('\nRemoving empty xml files')
    files_to_delete = []
    try:
        for file in [y for x in os.walk(folder) for y in glob(os.path.join(x[0], '*.xml*'))]:
            tree = None
            ...
            tree_root = tree.getroot()
            if len(tree_root.findall('./object')) == 0:
                files_to_delete.append(file)

        if len(files_to_delete) != 0:
            print('Empty files founded:', len(files_to_delete))
            if ask_user():
                print('DELETING:')
                for file in files_to_delete:
                    if os.path.isfile(file):
                        remove_from_folder(file)
                print('***   ' + len(files_to_delete) + ' Empty XML file found and deleted! ***')
        print('Removing is finished')
    except Exception:
        print('Unknown exception occurred. Folder:', folder)
