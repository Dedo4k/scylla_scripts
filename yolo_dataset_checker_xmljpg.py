...
predlabels = ['rifle', 'gun', 'gunarm', 'knife', 'knifearm', 'sniper', 'shotgun']
...
result = [y for x in os.walk(directory_in_str) for y in glob(os.path.join(x[0], '*.jpg'))] #Search for all mp4 files 
for files in result: 
    filename, file_extension = os.path.splitext(os.path.basename(files))
...
#     f1=f1.replace("___", "_")
#     f1=f1.replace("__", "_")
#     f1=f1.replace("americanboy", "risk")
